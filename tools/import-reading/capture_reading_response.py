...
# Chromium / Edge / Chrome Remote Debugging 地址。
DEBUGGER_URL = "http://127.0.0.1:9222/json"

# 当前已经确认：
# 全部 Reading Test 的列表请求名称中包含 lv2。
...
# 前两套保留少量诊断信息。
# 注意：不会打印任何 token / Authorization 值。
DEBUG_FIRST_TWO = True
...
def next_id():
    """
    每一个 Chrome DevTools Protocol 命令
    都需要一个唯一数字 id。
    """
    global message_id
    message_id += 1
...
def filter_reusable_headers(headers):
    """
    从真实 lv2 请求中提取可复用的应用层 Header。

    浏览器自己管理的 Header 不手动复制，例如：
    Cookie、Host、Origin、Referer、User-Agent、sec-* 等。

    这里只保留明显属于认证 / 应用自定义的 Header，
    例如：
    token、Authorization、X-Token、X-Custom-Auth。

    Header 的值只保存在当前 Python 进程内存中，
    不打印，也不会写入 raw / metadata / Git。
    """
    ...
    for name, value in headers.items():
        ...
        if (
            lower_name.startswith("sec-")
            or lower_name.startswith(":")
        ):
            continue

        # Reading 的真实请求除了 Authorization，
        # 还会携带 Source 和 Version。
        #
        # 如果缺少这两个 Header，
        # 服务端会返回：
        #
...
def capture_paper_list_response(ws):
    ...
    同时取得：
    1. lv2 Response Body -> 68 套 Test + paperId
    2. lv2 请求中的认证 Header -> 批量 Reading 请求复用
    """
    ...
    if "error" in response_body_result:
        raise RuntimeError(
    ...
    result = response_body_result.get(
        "result",
        {},
    )
...
def build_reading_api_url(paper_id):
    """
    根据 paperId 构造 Reading API。
    """
...
def fetch_reading_response(
    ...
    """
    使用真实 lv2 请求中已经存在的认证 Header，
    请求某一套 Reading。

    auth_headers 只存在当前脚本内存中。
    """
